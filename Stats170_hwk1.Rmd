...
```
Referring to the sample ACF plot, there are several autocorrelation that are outside the two blue bands, indicating that they are statistically significant.

```{r}
##### Finding the PACF of the random component
pacf(new_sales_ran, main = 'PACF of Random Component of Business Sales')
```

Referring to the sample ACF plot, AR model because there is a lot of 
autocorrelation that are significant. 
...
```{r}
ts.plot(diff(log(train[, 'biz_sales']),lg=1,differences = 1),
        main = 'Log of Total business sales in the U.S after differencing.',
        ylab = 'Log of Sales in millions of dollars after differencing')
#after doing the first difference, we see that the trend no longer exist
```
```{r}
ts.plot(diff(diff(log(train[, 'biz_sales']),lg=1,differences = 1),lag=12),
        main = 'Log of Total business sales in the U.S after differencing.',
        ylab = 'Log of Sales in millions of dollars after differencing')
